...
public:
    ...
    void helpWrite(int&count,std::string&input,int&current); // help write into file
    ...
    directory* findParent(directory* super,std::string name); // return the block* of the parent directory
    block* helpCreate(char type, std::string name);          // return the block* of newly allocated block
    directory* helpFind(directory* super, std::string name); // return the block* of the parent directory (overwrite)
