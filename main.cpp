...
int main() {
  ...
  while (strcmp(arg, "exit\n") != 0) {
    ...
    if (fgets(arg, ARGLEN, stdin) == NULL) {
      ...
      parse(arg, arglist);
      if ((strcmp(arglist[0], "create") == 0 ||
           strcmp(arglist[0], "CREATE") == 0) &&
          arglist[2] != nullptr && arglist[1] != nullptr) {
        Disk1->create(arglist[1][0], arglist[2]);
        Disk1->display();
      } else if ((strcmp(arglist[0], "open") == 0 ||
                  strcmp(arglist[0], "OPEN") == 0) &&
                 arglist[2] != nullptr && arglist[1] != nullptr) {
        Disk1->open(arglist[1][0], arglist[2]);
        Disk1->display();
      } else if (strcmp(arglist[0], "close") == 0 ||
                 strcmp(arglist[0], "CLOSE") == 0) {
        Disk1->close();
        Disk1->display();
      } else if ((strcmp(arglist[0], "delete") == 0 ||
                  strcmp(arglist[0], "DELETE") == 0) &&
                 arglist[1] != nullptr) {
        Disk1->deleteNew(arglist[1]);
        Disk1->display();
      } else if ((strcmp(arglist[0], "write") == 0 ||
                  strcmp(arglist[0], "WRITE") == 0) &&
                 arglist[2] != nullptr && arglist[1] != nullptr) {
        if (arglist[2][0] == '\'' &&
            arglist[2][std::strlen(arglist[2]) - 1] == '\'') {
      ...
      } else if ((strcmp(arglist[0], "read") == 0 ||
                  strcmp(arglist[0], "READ") == 0) &&
                 arglist[1] != nullptr) {
        Disk1->read(atoi(arglist[1]));
        Disk1->display();
      } else if ((strcmp(arglist[0], "seek") == 0 ||
                  strcmp(arglist[0], "SEEK") == 0) &&
                 arglist[1] != nullptr && arglist[2] != nullptr) {
